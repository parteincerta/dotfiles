...
if [ -d "$EXTERNAL_VOLUME" ]; then
	mkdir -p \
		...
		"$EXTERNAL_VOLUME/"{.docker,Misc,Recordings,Remote,Screenshots,Torrents,VMs}
	...
	ln -fs "$EXTERNAL_VOLUME/Recordings" "$DOCUMENTS"
	ln -fs "$EXTERNAL_VOLUME/Remote" "$DOCUMENTS"
	ln -fs "$EXTERNAL_VOLUME/Screenshots" "$DOCUMENTS"
